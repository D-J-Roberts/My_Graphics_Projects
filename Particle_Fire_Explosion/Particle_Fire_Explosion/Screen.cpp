...
namespace private_sdl {
	...
	void Screen::boxBlur() {
		...
		// Swap the buffers, so that the pixels are in m_buffer2 and we are drawing to m_buffer1.
		...
		for (int y = 0; y < SCREEN_HEIGHT; y++) {
			for (int x = 0; x < SCREEN_WIDTH; x++) {
				...
				// A pixel, 1, surrounded by eight other pixels, 0. For boxBlur FX take the average of all 9 colours and then use that colour for the central pixel.
				/*
				* 0 0 0
				* 0 1 0
				* 0 0 0
				*/
				...
				for (int row = -1; row <= 1; row++) {
					for (int col = -1; col <= 1; col++) {
						...
						// Check that the outer pixels don't go beyond the edge of the screen:
	...
	void Screen::setPixel(int x, int y, Uint8 red, Uint8 green, Uint8 blue) {
		...
		// Check to see if the pixels go beyond the screen edges:
